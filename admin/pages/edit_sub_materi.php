...
<?php include("partial/navbar.php") ?>
    ...
    <div class="container-fluid py-4">
      ...
      $materi = $_POST['materi'];
          $gas=mysqli_query($kon, "UPDATE tb_sub_materi SET id_materi='$materi',
          nama_sub_materi='$sub_materi' WHERE id_sub_materi='".$id."'");
          if($gas){
            ...
            echo"<script>
              ...
              window.location.href='edit_sub_materi.php?id=$id';
            </script>";
          }
          
        
      }else{
      $query = $kon->query("SELECT * FROM tb_sub_materi WHERE id_sub_materi=".$_GET['id']);
           $dt = mysqli_fetch_object($query);
        
    }
      ?>
      
      <form action="edit_sub_materi.php?id=<?=$dt->id_sub_materi?>"  method="POST" enctype="multipart/form-data">
        <div class="row my-2">
          <div class="col-md-2">
            <label for="exampleFormControlInput1">Materi</label>
          </div>
          <div class="col-md-5">
            <select class="form-control" name="materi" id="">
              ...
              ?>
              <?php
              if($dt->id_materi==$data['id_materi']){
              ?>
              <option value="<?= $data['id_materi'] ?>"  selected><?= $data['nama_materi'] ?></option>
              
              <?php
              continue;
              }
              ?>
              <option value="<?= $data['id_materi'] ?>"  ><?= $data['nama_materi'] ?></option>
        ...
        <div class="row my-2">
          <div class="col-md-2">
            <label for="exampleFormControlInput2">Sub Materi</label>
          </div>
          <div class="col-md-5">
            <input name="sub_materi" type="text" class="form-control" id="exampleFormControlInput2" value="<?= $dt->nama_sub_materi ?>">   
          </div>
        </div>
